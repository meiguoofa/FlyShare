...
魔法方法总是被**双下划线**包围，例如`__init__`。

魔法方法是面向对象的 Python 的一切，如果你不知道魔法方法，说明你还没能意识到面向对象的 Python 的强大。

魔法方法的“魔力”体现在它们总能够在**适当的时候被自动调用**。
...
- `__new__`对当前类进行了实例化，并将实例返回，传给`__init__`的`self`。但是，执行了`__new__`，**并不一定会**进入`__init__`，只有`__new__`返回了，当前类`cls`的实例，当前类的`__init__`才会进入。
...
- `__new__`方法主要是当你继承一些**不可变的 class** 时（比如`int, str, tuple`）， 提供给你一个自定义这些类的实例化过程的途径。
...
- `divmod(a, b)`把除数和余数运算结果结合起来，返回一个包含商和余数的**元组**`(a // b, a % b)`。
...
class Clockt_t:
    ...
    def __calc(self):
        ...
        for idx in range(len(self.unit)):
            #print(self.end[idx])
            self.lasted.append(self.begin[idx] - self.end[idx])
            if self.lasted[idx]:
                prompt += str(result[idx]) + "".join(self.unit[idx])
    ...
    def __add__(self, other):
        ...
        for idx in range(len(self.unit)):
            #print("xxx")
            result.append(self.lasted[idx] + other.lasted[idx])
                #print("xxx")
            prompt += str(result[idx]) +"  "+ "".join(self.unit[idx]) +"  "
